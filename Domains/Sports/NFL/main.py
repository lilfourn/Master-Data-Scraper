#!/usr/bin/env python3
"""
NFL Domain Scraper CLI

This script provides a command-line interface for scraping NFL statistics
...
from Domains.Sports.NFL.scraper import NFLScraper
...
@click.pass_context
def cli(ctx):
    """NFL Statistics Scraper - Get NFL data from multiple sources"""
    ctx.ensure_object(dict)
    ctx.obj['scraper'] = NFLScraper()
    
    # Show banner
    banner = Panel(
        "[bold cyan]NFL Statistics Scraper[/bold cyan]\n"
        "[dim]Domain: Sports/NFL[/dim]\n"
...
@cli.command()
@click.option('--conference', '-c', type=click.Choice(['AFC', 'NFC']), 
...
def standings(ctx, conference, format):
    ...
    with console.status("[bold yellow]Fetching NFL standings..."):
        try:
...
def scores(ctx, date, team, format):
    ...
    with console.status("[bold yellow]Fetching NFL scores..."):
        try:
...
@click.option('--position', '-P', type=click.Choice(['QB', 'RB', 'WR', 'TE', 'DEF']),
...
def scrape_all(ctx, data_types, format):
    """Scrape all or specified NFL data types"""
    scraper = ctx.obj['scraper']
...
def sources(ctx):
    ...
    sources_table = Table(title="NFL Data Sources", box=box.ROUNDED)
